...
p. NivelStyle is a CSS framework that focus in:
    ...
    * Use flexible estructures without need grids
    * Reuse always the same classes to build your layout
    * Build structures withour redundant code that will be rewriten
...
* *reset.css*: This is reset archive created by Eric Meyer. It isn't required to use it, you can use the reset.css file you prefer.
* *nivelstyle_form.css*: Styles to build a form. In form.html you will find de structure.

* *nivelstyle.css*: Layout styles. With this styles you can build all the structures you want.
* *estructuras.css*: Your styles. You can choose this name or include the stylesheet you are working on.

h2. How to use it

p. NivelStyle is half HTML y half CSS, you need both to create flexible strcutures and easy to control.
...
fn1. It is not necessary incluide main.css, if you only want to use the layout code, you can call only nivelstyle.css. If you will use NivelStyle in a builded project, verify that your classes are not using the same class names. Because this may alter the functioning of the style sheet nivelstyle.
